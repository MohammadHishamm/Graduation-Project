...
class JavaCouplingBetweenObjects extends MetricCalculator_1.MetricCalculator {
    ...
    calculateClassCBO(currentClass, allClasses, allMethods, allFields) {
        ...
        const classMethods = allMethods.filter((method) => {
            ...
            return allClasses.some((cls) => cls.name === currentClassName &&
            ...
            method.methodCalls.forEach((methodCall) => {
                ...
                if (parts.length > 1) {
                    ...
                    const matchingField = allFields.find((field) => {
                        const fieldBelongsToClass = allClasses.some((cls) => cls.name === currentClassName &&
                    ...
                    if (matchingField && isExternalClass(matchingField.type)) {
                        ...
                        console.log(`[CBO] Found call to external class through field: ${objectName} (${matchingField.type})`);
                    }
                }
            });
        });
        ...
        classMethods.forEach((method) => {
            method.fieldsUsed.forEach((fieldName) => {
                ...
                const matchingField = allFields.find((field) => field.name === fieldName && isExternalClass(field.type));
                if (matchingField) {
                    const baseType = matchingField.type.split("<")[0];
                    externalDependencies.add(baseType);
                    console.log(`[CBO] Found access to external field: ${fieldName} (${baseType})`);
                }
            });
            ...
            method.localVariables.forEach((localVar) => {
                ...
                if (isExternalClass(varType)) {
                    ...
                    externalDependencies.add(baseType);
                    console.log(`[CBO] Found local variable of external type: ${baseType}`);
                }
            });
        });
        ...
        classFields.forEach((field) => {
            if (isExternalClass(field.type)) {
                ...
                externalDependencies.add(baseType);
                console.log(`[CBO] Found field of external type: ${field.name} (${baseType})`);
            }
        });
        ...
        console.log(`[CBO] Related classes (self + ancestors): ${Array.from(relatedClasses)}`);
        ...
        console.log(`[CBO] External dependencies for ${currentClassName}: ${Array.from(externalDependencies)}`);
        console.log(`[CBO] Final CBO value for ${currentClassName}: ${cboValue}`);
        return cboValue;
    }
    getAncestorClasses(currentClass, allClasses) {
        ...
        if (currentClass.parent) {
            ...
            const parentClass = allClasses.find((cls) => cls.name === currentClass.parent);
            if (parentClass) {
                const parentAncestors = this.getAncestorClasses(parentClass, allClasses);
